...
var GHOST_MINKY_TRAILER_POSITION_X = 1105;
var GHOST_MINKY_TRAILER_POSITION_Y = 25;
var GHOST_MINKY_TRAILER_DIRECTION = 3;
var GHOST_MINKY_TRAILER_COLOR = "#f99c00";
var GHOST_MINKY_TRAILER_BODY_STATE = 3;
var GHOST_MINKY_TRAILER_STATE = 0;
...
function initHome() { 
	...
	canvas = document.getElementById('canvas-presentation-minky');
	canvas.setAttribute('width', '50');
	canvas.setAttribute('height', '50');
	if (canvas.getContext) { 
		ctx = canvas.getContext('2d');
	}
	ctx.fillStyle = GHOST_MINKY_COLOR;
	drawHelperGhost(ctx, 25, 25, 1, 0, 0, 0);
...
function nextSequencePresentation() { 
	if (HOME_PRESENTATION_STATE === 0) { 
		...
		$("#canvas-presentation-minky").show();
	} else if (HOME_PRESENTATION_STATE === 16) { 
		$("#presentation-character-minky").show();
	} else if (HOME_PRESENTATION_STATE === 17) { 
		$("#presentation-name-minky").show();
	}
...
function nextSequenceTrailer() { 
	...
	if (GHOST_MINKY_TRAILER_BODY_STATE < GHOST_TRAILER_BODY_STATE_MAX) { 
		GHOST_MINKY_TRAILER_BODY_STATE ++;
	} else { 
		GHOST_MINKY_TRAILER_BODY_STATE = 0;
	}				
	...
	if ( GHOST_MINKY_TRAILER_DIRECTION === 1 ) { 
		GHOST_MINKY_TRAILER_POSITION_X += GHOST_TRAILER_POSITION_STEP;
	} else if ( GHOST_MINKY_TRAILER_DIRECTION === 3 ) { 
		GHOST_MINKY_TRAILER_POSITION_X -= GHOST_TRAILER_POSITION_STEP;
	}
	...
	if ( GHOST_MINKY_TRAILER_POSITION_X < -150) { 
		GHOST_MINKY_TRAILER_DIRECTION = 1;
		GHOST_MINKY_TRAILER_STATE = 1;
	}
...
function drawGhostsTrailer() { 
	...
	if (GHOST_MINKY_TRAILER_STATE === 1) { 
		ctx.fillStyle = GHOST_AFFRAID_COLOR;
	} else { 
		ctx.fillStyle = GHOST_MINKY_COLOR;
	}
	drawHelperGhost(ctx, GHOST_MINKY_TRAILER_POSITION_X, GHOST_MINKY_TRAILER_POSITION_Y, GHOST_MINKY_TRAILER_DIRECTION, GHOST_MINKY_TRAILER_BODY_STATE, GHOST_MINKY_TRAILER_STATE, 0);
}
function eraseGhostsTrailer(ghost) { 
	...
	ctx.clearRect(GHOST_MINKY_TRAILER_POSITION_X - 17, GHOST_BLINKY_TRAILER_POSITION_Y - 17, 34, 34);
}
